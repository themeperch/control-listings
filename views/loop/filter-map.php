    ...
    <div class="search-listings-wrapper listings-search-form d-flex flex-wrap gap-30 bg-white p-20 mb-20 border-bottom">
        <div class="input-group gap-2">
            <div class="form-floating">
                <input type="text" name="terms" value="<?php echo get_query_var('terms'); ?>" placeholder="<?php esc_attr_e('Search keywords', 'control-listings') ?>" id="listingSearchInput" class="form-control" />
                <label for="listingSearchInput"><?php esc_attr_e('Search keywords', 'control-listings') ?></label>
            </div>
            <div class="form-floating">
                <input id="listingSearchAge" type="text" name="age" class="form-control"  value="<?php echo get_query_var('age'); ?>" placeholder="<?php esc_attr_e('Age: (in Years)', 'control-listings') ?>" pattern="[0-9]" />
                <label for="listingSearchAge"><?php esc_attr_e('Age: min-max (Years)', 'control-listings') ?></label> 
            </div>
            <div class="form-floating">
                <input id="listingSearchPrice" type="text" name="price" class="form-control" placeholder="<?php /* translators: %s is the currency symbol. */ printf(__('Price: (in %s)', 'control-listings'), get_control_listings_currency()) ?>" value="<?php echo get_query_var('price'); ?>"  pattern="[0-9]" />
                <label for="listingSearchPrice"><?php /* translators: %s is the currency symbol. */ printf(__('Price: (%s)', 'control-listings'), get_control_listings_currency()) ?></label> 
            </div>
            <?php
            ...
            <select class="form-select category-select" name="lcat">
                <option value=""><?php echo __('Category', 'control-listings') ?></option>
