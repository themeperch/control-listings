...
$bookmarked_text = $is_bookmarked ? __( 'Update Bookmark', 'control-listings' ) : __( 'Add Bookmark', 'control-listings' );
?>
<form method="post" action="<?php echo defined( 'DOING_AJAX' ) ? '' : esc_url( remove_query_arg( array( 'page', 'paged' ), add_query_arg( $wp->query_string, '', home_url( $wp->request ) ) ) ); ?>" class="control-listings-form control-listings-bookmarks-form <?php echo esc_attr($bookmarked_class) ?>">
	<div class="remove-bookmark-wrapper"><a class="remove-bookmark" href="<?php echo wp_nonce_url( add_query_arg( 'remove_bookmark', absint( $post->ID ), get_permalink() ), 'remove_bookmark' ); ?>"><?php _e( 'Remove Bookmark', 'control-listings' ); ?></a></div>
	
	<div class="bookmark-details">
		<p><label for="bookmark_notes"><?php _e( 'Notes:', 'control-listings' ); ?></label><textarea name="bookmark_notes" id="bookmark_notes" cols="25" rows="3"><?php echo esc_textarea( $note ); ?></textarea></p>
		<p>
