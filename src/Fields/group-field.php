...
class RWMB_Group_Field extends RWMB_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		foreach ( $fields as $field ) {
			if ( $field['clone'] ) {
				wp_enqueue_script( 'rwmb-clone', CTRL_LISTINGS_ASSETS . 'fields/clone.js', [ 'jquery-ui-sortable' ], RWMB_VER, true );
				break;
			}
		}
		...
		wp_enqueue_style( 'rwmb-group', CTRL_LISTINGS_ASSETS . 'fields/group.css', [], time() );
		wp_enqueue_script( 'rwmb-group', CTRL_LISTINGS_ASSETS . 'fields/group.js', [ 'jquery', 'underscore', 'wp-hooks' ], time(), true );
		wp_localize_script( 'rwmb-group', 'RWMB_Group', [
			/* translators: %s: Field name  */
			'confirmRemove' => __( 'Are you sure you want to remove %s?', 'control-listings' ),
			'defaultTitle'  => __( 'this group', 'control-listings' ),
			'on'            => __( 'On', 'control-listings' ),
			'off'           => __( 'Off', 'control-listings' ),
			'yes'           => __( 'Yes', 'control-listings' ),
			'no'            => __( 'No', 'control-listings' ),
		] );
	}
	...
	public static function normalize( $field ) {
		...
		$field = wp_parse_args( $field, [
			'collapsible'   => false,
			'save_state'    => false,
			'group_title'   => $field['clone'] ? __( 'Entry {#}', 'control-listings' ) : __( 'Entry', 'control-listings' ),
