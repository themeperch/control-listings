...
class Geolocation {
	...
	public function enqueue( $meta_box ) {
		...
		wp_enqueue_script( 'mb-geolocation', CTRL_LISTINGS_ASSETS . '/fields/geolocation.js', ['google-maps', 'jquery', 'underscore'], time(), true );
	}
	...
	private function guess_binding_field( $field_id ) {
		$available_fields = array(
			...
			'reference', 'url', 'vicinity', 'geometry', 'website', 'formatted_phone_number', 'international_phone_number', 
     ...
     */
	private function check_type_google_map( $meta_box ) {
		$is_map = false;
		...
		if ( false !== $is_field_group ) {
			$is_map = $this->check_type_google_map( $meta_box['fields'][ $is_field_group ] );
		}

		if ( false === $is_map ) {
			$is_map = array_search( 'map', array_column( $meta_box['fields'], 'type' ) );
		}
